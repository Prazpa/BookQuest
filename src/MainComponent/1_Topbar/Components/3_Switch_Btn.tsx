...
const Switch_Btn = () => {
    ...
    return (
        <div className={`flex items-center space-x-2 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`} onClick={toggleDarkMode}>
            ...
            <Label htmlFor="darkModeSwitch" className={`sm:space-x-1 text-[14px] font-bold  ${darkMode ? 'hover:text-[#FF5A67]': 'hover:text-[#2DD4C5]'}`}>{darkMode ? "Light mode" : "Dark mode"}</Label>
        </div>
    );
}
